...
//        Customer user = new Customer();
//
//        Mockito.when(usersService.updateProfile(eq(id), any(UsersRequestDTO.class))).thenReturn(user);
//
//        try (MockedStatic<UsersMapper> mockedMapper = Mockito.mockStatic(UsersMapper.class)) {
//            mockedMapper.when(() -> UsersMapper.toDTO(user)).thenReturn(responseDTO);
//
//            mockMvc.perform(put("/api/users/{email}", id)
//                    .contentType(MediaType.APPLICATION_JSON)
//                    .content(objectMapper.writeValueAsString(requestDTO)))
//                    .andExpect(status().isOk())
//                    .andExpect(jsonPath("$.email").value(id))
